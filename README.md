...
This Express, TypeScript, MongoDB, and React bank application includes JWT
authentication, account ownership checks, MongoDB ObjectIds, atomic money
operations, idempotency protection, transaction history, and a routed MUI
frontend.

This README is also a review guide. It explains the data and API contracts,
why the security and consistency fields exist, and what reviewers should verify.

## Important database note

Use an empty database that follows the ObjectId schema documented below:

```env
MONGODB_DB=bankapp_auth
```

The startup process does not migrate legacy numeric IDs or money fields. A
separate database name in the same Atlas project is sufficient for review.
...
Review against the empty database recommended above so every document uses the
current ObjectId schema.
